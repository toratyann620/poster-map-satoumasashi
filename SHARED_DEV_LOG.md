...
### 2026-08-29 (Claude Code) その44
* **タスク**: Phase 7（データ移行・切替）の実施。モバイルはアプリへ移行し、PCはブラウザ版を継続、両者でデータを共通化する。
* **切替の順序**（①と③の間にWebで編集が入ると取り残されるため、続けて実施した）:
  1. `node scripts/migrate_to_v2.mjs` — v1→v2 の差分同期
  2. `feature/groups-admin` → `main` のマージを確定（`37b778e`）、push
  3. `vercel deploy --prod` — Webを v2 参照へ切替
  4. `firebase deploy --only functions` — 日次レポートも v2 へ
* **同期結果**: `posters` 1462件 = `posters_v2` 1462件で一致。`activityLogs_v2` は1477件で v1 より6件多いが、**すべて 2026/8/24 の権限検証時に検証用アカウントが残したログ**で、実データではないことを確認済み。
* **本番Webの反映確認**（バンドル `index-BS7_EU5g.js`）: v2への切替 / チュートリアル / お知らせ / 初期パスワード変更 / ダッシュボードの内訳表示 / ウィジェットの撤去済み除外（圧縮後のコードで `type==="佐藤まさし"&&!A.removed` を確認）— いずれも反映済み。
* **アプリの配信**: 1.0.2 / build 4 へ更新し、チュートリアル・お知らせ・プッシュ通知・初期パスワード変更を含む版を作成。
  * **Android**: 内部テスト = `completed`（配信中）、クローズドテスト = `draft`。**クローズドテストは Play Console からの公開操作が必要**（初回公開前のアプリは内部テスト以外 draft でしか作成できないため）。
  * **iOS**: `build/upload/postermap-1.0.2-build4.ipa` を TestFlight へアップロード成功（`UPLOAD SUCCEEDED with no errors`）。Apple側の処理完了を待って TestFlight に表示される。
* **注意**:
  * 旧コレクション（`posters` / `activityLogs`）は残しているが、以降は更新されない。参照しているものが無いことを確認済み（日次レポートは同時に v2 へ向けた）。
  * Webにもグループ権限がかかるようになった。9名全員が有効なグループに所属済みで締め出しは発生しない。
  * 市区町村を判定できないポスターが2件ある（住所が「シャトルＡＺＵＭＡ」「コスモ」と建物名のみ）。近隣ピンからいずれも海老名市とみられる。**現状は佐藤まさし事務所からしか見えない**ため、管理画面の「市区町村の手当て」から住所を補完すること。綾瀬市の2件も、どのグループの担当条件にも含まれない。
  * 検証用アカウントが3つある（`qa.verification` / `test.nanba` / `test.nanba2`）。確認完了後に削除する。

### 2026-08-30 (Claude Code) その45
* **タスク**: 作業の依頼（タスク）機能、マイページ、ユーザー管理の管理画面完結
* **ユーザー確定事項**: ①タスクはポスターに紐付ける（任意）②タスクはグループ単位で分ける ③ユーザー削除でログインアカウントごと消す
* **設計判断**:
  * **タスクはグループ単位で分離**。ポスターと同じ区切りにしないと「依頼は見えるのに対象のピンは開けない」状態が起きる。佐藤まさし事務所（allowAll）は全事務所の依頼を扱える。ルールの分岐が `resource.data` を参照しないため、条件を付けない一覧取得も通る（posters_v2 と同じ作法）。
  * **担当者未指定＝事務所の全員あて**。個人タスクと共通タスクを別コレクションに分けず、`assigneeUid` の有無で表す。手が空いた人が拾う種類の依頼を、仕組みを増やさずに表せる。
  * **完了は事務所の誰でも操作できる**。担当者以外が代わりに済ませることがあるため。誰が済ませたかは `completedBy` に残す。
  * **`groupId` の付け替えはルールで禁止**。依頼ごと別の事務所へ移せてしまうため。
  * **マイページは自分あてと全体あてを同じ画面に出す**。後者を別画面にすると、手が空いた人が拾いに行かなくなる。
  * **アカウント削除は Cloud Function（`deleteUserAccount`）で受ける**。クライアントSDKでは他人の Auth アカウントを消せない。呼べるのは佐藤まさし事務所の管理者のみで、`users` の書き込み条件と揃えてある。pushTokens も先に消す（アカウントが消えた後だと持ち主を辿れないため）。
* **追加したもの**: `tasks` コレクション / `sendTaskPush`・`deleteUserAccount`（Cloud Functions）/ `src/hooks/useTasks.ts` / `src/components/MyPage.tsx` / `src/admin/TasksTab.tsx`。モバイル管理パネルにも権限・所属の変更を追加した（従来は作成と削除のみ）。
* **検証**:
  * ルール境界テスト **63→74件、全通過**。「他事務所の依頼は読めない」「他事務所あては作れない」「別の事務所へ付け替えられない」「一般ユーザーは削除できない」を確認。
  * **複合インデックスが必要だった**（`tasks`: groupId × createdAt）。実画面で `The query requires an index` が出て判明。追加・デプロイ済み。エミュレータのルールテストでは検出できない類のため、実データでの確認が要る。
  * 難波事務所のアカウントで実画面を確認。未対応バッジ1件、依頼タブに依頼が表示、「地図で見る」あり、完了済みタブにアーカイブ表示、作業履歴タブも表示。確認用に入れた依頼2件は削除済み（現在 tasks は0件）。
  * `tsc` / `build` 通過、lint 35→34件。
* **本番反映済み**: ルール・インデックス・Cloud Functions（`sendTaskPush` と `deleteUserAccount` を新規作成）。
* **未完了 / 次のステップ**:
  1. **Web・アプリへの配信が未実施**。コミット `ec88ed7`。ブラウザ版は `vercel deploy --prod`、アプリは 1.0.3 へ上げて再ビルドが必要。
  2. **プッシュ通知の実機確認が未実施**。依頼を作ると担当者（または事務所の全員）へ届くはずだが、実機でしか確かめられない。
  3. 直前の改修（ピンの町域集計、コミット `1e979d0`）も未配信。まとめて配信できる。

### 2026-09-01 (Claude Code) その46
* **タスク**: 撤去理由・Googleマップ連携・ピン打ちモードの実装、シミュレータでのUI確認と改善、ブラウザ版とアプリの配信
* **実装**:
  * **撤去理由**: 撤去時に理由を尋ねる（任意）。必須にすると現地で手が止まり、そもそも撤去の記録が残らなくなるため。撤去済みを表示する設定のとき、詳細と折りたたみ時のバッジから確認できる。撤去を取り消すと理由も消す。
  * **Googleマップ連携**: ピンの詳細から、その地点を目的地として開く。
  * **ピン打ちモード**: よく使う登録内容を3つまで決めておき、現在地にボタンひとつでピンを立てる。位置は押すたびに取り直す（直前の現在地を使い回すと、歩きながら続けて登録したとき前の地点に立つ）。住所は逆引きするが取れなくても登録は止めない。登録内容は端末ごとに保存する。
* **シミュレータ（iPhone 17）で見つけて直した4点**:
  1. **新規ピンの詳細シートが検索窓に被る**（ユーザー報告）。iPhone 17 の寸法で測ったところ、3行の検索窓が 75〜219pt を占め、展開したシートの上端131ptに **88pt 食い込んでいた**。シートが展開しているあいだは検索窓を隠すようにした。
  2. **一般ユーザーがピン打ちモードを設定できなかった**。登録内容の編集を管理パネルの中だけに置いていたが、ピン打ちモードは現場で使う機能で使うのは主に一般ユーザー。編集画面を切り出し、ピン打ちモードの帯と案内から開けるようにした。
  3. 帯に設定ボタンを足したことで「ピン打ちモード」が2行に折り返して崩れていた。
  4. **ボタンが大きすぎる**（ユーザー指摘）。「ここに『◯◯』を立てる」の横長ボタン3つで地図の下半分が埋まっていた。番号だけの丸（56px）を横に並べる形に変更し、登録内容から「ボタンの名前」も外した。
* **シミュレータ操作の知見**:
  * クリック座標は、ウィンドウのサイズから単純計算しても当たらない。タイトルバーと端末のベゼルが含まれるため。ウィンドウを撮影して画面領域の端を実測し、換算式を出した（`scratchpad/tap.sh` にコメント付きで残した）。
  * **日本語の入力はできない**。`keystroke` は IME を経由せず、ペーストボード経由でも文字化けする。テキスト入力を伴う確認はその部分だけ手動が要る。
  * アクセシビリティの許可（システム設定 → プライバシーとセキュリティ）が無いと `System Events` からの操作が一切できない。
* **実機での確認**: ログイン → プッシュ通知の許可 → 新規ピンのシート（重なりなし）→ ピン打ちモードで登録内容を追加 → ボタンを押して現在地にピンが立つところまで通した。立ったピンは住所が逆引きされ（〒243-0018 神奈川県厚木市中町3-17-17）市区町村も厚木市と判定され、難波事務所のアカウントでもグループ権限を満たして書き込めていた。**確認に使ったピンと変更履歴は削除済み**。
* **配信**:
  * ブラウザ版: デプロイ済み（バンドル `index-CtFS8_6p.js`）。
  * **iOS 1.0.3 (build 5)**: TestFlight へアップロード成功。
  * **Android 1.0.3 (versionCode 5)**: 内部テスト = `completed`（配信中）、クローズドテスト = `draft`（Play Console からの公開操作が必要）。
  * `scripts/upload_play.mjs` の不具合を修正: リリース名を指定していなかったため、同じトラックに残っていた下書きの名前（1.0.2）が引き継がれ、Play Console に古いバージョンとして表示されていた。名前を明示するようにし、今回の分も 1.0.3 (5) に直した。
* **未完了**:
  1. **Android のクローズドテストが下書きのまま**。Play Console からの公開操作が要る。
  2. **プッシュ通知の実機確認**。シミュレータでは許可ダイアログまでしか確認できていない。実機で依頼を作って届くかの確認が要る。
  3. 検証用アカウント3つ（`qa.verification` / `test.nanba` / `test.nanba2`）の削除。

### 2026-09-02 (Claude Code) その47
* **タスク**: ユーザー報告の3点（ピン打ちが30秒かかる／現在地が移動に追従しない／撤去理由が表示されない）の修正
* **① ピン打ちの待ち時間（30秒 → 0.8秒）**
  * **原因**: ボタンを押すたびに `getCurrentPosition` で GPS を取り直していた。`enableHighAccuracy: true` かつ `maximumAge: 5000` のため、常時 `watchPosition` が動いていても毎回ゼロから測位していた（timeout は15秒）。
  * **対処**: 監視中の現在地をそのまま使う。一発取得側も `maximumAge` を30秒に緩めた（`ONESHOT_OPTIONS`）。
* **② 現在地の追従**
  * **原因**: 起動時の1回しか `setMapCenter` していなかった。
  * **対処**: Googleマップと同じ考え方で、追従中は測位のたびに地図を動かす。地図のドラッグ（`dragstart`）で解除、現在地ボタンで再開。検索・依頼からピンへ飛ぶ等の意図的な移動でも解除する（でないと次の測位で引き戻される）。追従中は現在地ボタンを塗って状態が分かるようにした。
  * ⚠️ Map.tsx には**ナビ用の `isFollowing` が既にある**ため、現在地追従の prop は `followingLocation` という別名にしてある。
* **③ 撤去理由**
  * **原因は2つ**。保存側は `window.prompt` を使っており、実データを見ると**撤去済み73件すべてで `removalReason` フィールド自体が存在しなかった**（WebView で動いていない）。表示側は `sheetState === 'expanded'` の条件付きだったが、既存ピンは折りたたみで開くため出ていなかった。
  * **対処**: 画面内のダイアログ（`RemovalDialog.tsx`）に置き換え、よく使う理由を選べるようにした。表示は撤去済みなら常に詳細の先頭に出す。
* **調査中に見つけた別の不具合（重要）**
  * **Maps API を言語指定なしで読み込んでいた**。端末が英語設定だと逆引きが `Atsugi` のように返る。`city` はグループ権限の判定に使うため、書き込みが拒否されたり、allowAll のアカウントでは**誤った city のまま保存されて範囲判定が狂う**恐れがあった。`<Wrapper ... language="ja" region="JP">` を指定して固定。既存データに英語表記の混入が無いことは確認済み（厚木市609 / 海老名市476 / 伊勢原市375 / 綾瀬市2 / 空2）。
  * **逆引きに失敗すると city が空になり、書き込み自体が拒否される**。現場では「なぜか登録できない」状態になる。最寄り3km以内のピンから市区町村を借りる手当てを入れ、それでも決まらない場合は理由を伝えて中断する。
* **検証**:
  * iPhone 17 シミュレータで現在地を1.2km動かし、地図が追従することを確認。
  * ブラウザ（402×874）でピン打ち **0.8秒**・書き込みの拒否なし。作られたピンは `city=厚木市`（日本語）で保存されていた。
  * 撤去済みのピンで「撤去の理由」が詳細の先頭に出ることを確認。
  * 確認に使ったピンと履歴は削除済み。
* **配信**: ブラウザ版デプロイ済み。**iOS 1.0.4 (build 6)** TestFlight へアップロード成功、**Android 1.0.4 (versionCode 6)** 内部テスト配信中・クローズドテストは下書き。
* **注意（作業手順）**: `release_ios.sh` と `release_android.sh` を**同時に走らせてはいけない**。両方が `npm run build` で `dist` を作り直すため、片方の `cap sync` が「dist に index.html が無い」で失敗する。実際に1度失敗した。
* **未完了**: プッシュ通知の実機確認、Android クローズドテストの公開操作、検証用アカウント3つの削除。

### 2026-09-03 (Claude Code) その48
* **タスク**: 種類「連絡所(佐藤まさし)」の追加と25件の一括登録、依頼機能の開放、完了結果、右上の事務所名表示、1.0.5 の配信
* **① 種類の追加と一括登録**
  * 「連絡所(佐藤まさし)」を **#F97316（オレンジ）** で追加。既存11色と重ならず、かつ佐藤まさしの青（#3B82F6）と地図上で紛れない色を選んだ。
  * 25件（厚木12 / 海老名8 / 伊勢原5）を登録。**全件が番地レベル（ROOFTOP）で特定できた**。ステータス「設置済」・枚数1・備考8件、変更履歴も残した。
  * **誤字とみられる3件を補正**: 「厚市金田478」→厚木市 / 「大谷来た1-5-27」→大谷北 / 「海者名市大谷南3-21-22」→海老名市。
  * ⚠️ **他事務所からはこの種類が見えない**（各グループの `types` に入れていないため）。見せる場合は管理画面のグループ管理で追加する。
* **② 依頼を全ユーザーが出せるように**
  * マイページに「作業を依頼する」を追加。現場で気づいたことをその場で依頼にできないと、結局メモ止まりになるため。
  * 担当者を選ぶには在籍者の名前が要るので、**`users` の閲覧に「同じ事務所のメンバー」を追加**した。他事務所の在籍者は引けず、書き換えも従来どおりできない。
  * ⚠️ セキュリティルールの緩和にあたるため、不要なら「担当者を指定しない依頼のみ」に戻せる。
* **③ 完了の結果**: 完了時に任意で結果を残せるようにし、マイページと管理画面の完了済み一覧に表示。未対応に戻すと結果も消す。
* **④ 見つけた不具合**: **依頼の作成が失敗していた。** Firestore は `undefined` を受け付けないため、対象のポスターや担当者を決めない依頼が「Unsupported field value: undefined」で丸ごと失敗していた。管理画面からの作成も同じ経路。`createTask` で undefined を落としてから書き込むようにした。
* **⑤ 右上に事務所名**: どのグループで見ているかが分からないと、ピンが少ないときに「絞り込まれている」のか「本当に無い」のかを判断できないため。当初は検索窓に内側の余白を足しただけで、白い箱の中にバッジが乗って見えていた（ユーザー指摘）。箱自体を短くして横に並べた。
* **検証**: ルール境界テスト **74→79件、全通過**。実画面で「マイページから依頼を作成 → 担当者に同じ事務所の2名が出る → 完了ダイアログで結果を入力 → 完了済みタブに表示」まで通した。確認用のデータは削除済み。
* **配信**: ブラウザ版デプロイ済み。**Android 1.0.5 (versionCode 7)** 内部テスト配信中・クローズドテストは下書き。**iOS 1.0.5 (build 7)** アップロード中。
* **未完了**: Android クローズドテストの公開操作、プッシュ通知の実機確認、検証用アカウント3つの削除。

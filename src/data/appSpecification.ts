...
export const APP_SPECIFICATION: SpecSection[] = [
    ...
    },
    {
        ...
        blocks: [
            {
                type: 'paragraph',
                text: '管理者専用の分析ツールです。日付レンジ（デフォルト: 本日から30日前）およびステータスフィルターでの絞り込みが可能です。'
            },
            {
                type: 'subheading',
                text: '■ ダッシュボードタブ'
            },
            {
                type: 'list',
                items: [
                    '佐藤まさし KPI: 「佐藤まさし」に特化した総ピン数、設置済み率、および期間中の純増減数を表示します。',
                    '期間アクション総数: 期間中に実行された追加・更新・削除の件数をカウントします。',
                    '最終更新日時 & 最も活動的な曜日の自動算出。',
                    '種類別ピン数推移折れ線グラフ: 各種類ポスターの期間中の累積ピン数の増減推移を折れ線グラフ (SVG) で可視化します。',
                    '日別アクション推移グラフ: 毎日のアクション数（追加・更新・削除）を積み上げ棒グラフで表示します。',
                    '種類別サマリーテーブル: 種類ごとの現在数、設置率、追加・削除数、純増減を表形式で一覧化します。'
                ]
            },
            {
                type: 'subheading',
            ...
            },
            {
                type: 'list',
                items: [
                    '活動ランキング: 期間中のアクション数が最も多いユーザーをランキング順に表示し、各ユーザーの登録ピン数や主要なポスター種類を表示します。',
